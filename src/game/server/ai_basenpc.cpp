...
int CAI_BaseNPC::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	if ( m_flLastDamageTime != gpGlobals->curtime )
	{
		...
		else
		{
			...
			//MODIFICATION HERE!
			//if attacker alive and exists and player exists do sum shit
			if( pAttacker && pAttacker->IsAlive() && pPlayer )
			{
				//check if attacker is in plr squad
...
void CAI_BaseNPC::DrawDebugGeometryOverlays(void)
{
	...
	if (m_debugOverlays & OVERLAY_NPC_KILL_BIT) 
	{
		...
		info.SetInflictor( ( AI_IsSinglePlayer() ) ? (CBaseEntity *)UTIL_GetNearestPlayer(this) : (CBaseEntity *)this );
...
CBaseEntity *CAI_BaseNPC::FindNamedEntity( const char *name, IEntityFindFilter *pFilter )
{
	...
	{
		return ( CBaseEntity * )UTIL_GetNearestPlayer(this);
	}
...
{
	CBasePlayer *pPlayer = UTIL_GetNearestPlayer(pNPC);
	QAngle fan;
...
bool CAI_BaseNPC::FindNearestValidGoalPos( const Vector &vTestPoint, Vector *pResult )
{
	...
	if ( vCandidate != vec3_invalid )
	{
		AI_Waypoint_t *pPathToPoint = GetPathfinder()->BuildRoute( GetAbsOrigin(), vCandidate, UTIL_GetNearestPlayer(this), 5*12, NAV_NONE, true );
		if ( pPathToPoint )
		{
